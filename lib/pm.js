...
exports.install = function(pm, options) {
    ...
    return exports.path(pm).then(function(path) {
        ...
        function installGlobal(cachePath, status) {
            ...
            if (status === 304 && PATH.existsSync(cachePath)) {
                return Q.call(function() {
                    return cachePath;
                });
            }
            ...
            return NPM.install(cachePath, ".").then(function() {
                ...
                if (typeof options.name !== "undefined" && descriptor.name !== options.name) {
        ...
        function install(cachePath, status) {
            return installGlobal(cachePath, status).then(function(cachePath) {
                ...
                if (PATH.existsSync(path)) {
                    ...
                    TERM.stdout.writenl("\0cyan(" + "Backing up '" + path + "' to '" + backupPath + "'." + "\0)");
                ...
                return FS_RECURSIVE.osCopyDirRecursive(cachePath, path).then(function() {
                    ...
                    if (PATH.existsSync(PATH.join(path, ".git"))) {
                        TERM.stdout.writenl("\0cyan(Deleting git version control for package '" + path + "' to put it into read only mode.\0)");
