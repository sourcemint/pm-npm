...
exports.install = function(pm, options) {
    ...
    return exports.path(pm).then(function(path) {
        ...
        if (/^git(@|:\/\/)/.test(url)) {
            ...
            TERM.stdout.writenl("\0cyan(Cloning git repository '" + urlInfo.path + "' into '" + path + "'\0)");
            
            if (PATH.existsSync(path)) {
                FS_RECURSIVE.rmdirSyncRecursive(path);
            }
            
            return git.clone(urlInfo.path).then(function() {
                if (!urlInfo.hash) {
                    return install(".");
                }
                
                var ref = urlInfo.hash.substring(1);

                TERM.stdout.writenl("\0cyan(Checking out tag: " + ref + "\0)");

                return git.checkout(ref).then(function() {
                    return install(".");
                });
            });
        }
